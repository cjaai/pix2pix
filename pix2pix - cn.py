...
#参数 type、default、required、help、action  comment by  Tony
#store_true 存储为True
...
#add by Tony
parser.add_argument("--input_file", required=True,help="input images") 
parser.add_argument("--output_file", required=True,help="output images") 
...
#Discriminator卷积层
#pad的四个维度：[batch, in_height, in_width, in_channels]
def discrim_conv(batch_input, out_channels, stride):
    padded_input = tf.pad(batch_input, [[0, 0], [1, 1], [1, 1], [0, 0]], mode="CONSTANT")
    return tf.layers.conv2d(padded_input, out_channels, kernel_size=4, strides=(stride, stride), padding="valid", kernel_initializer=tf.random_normal_initializer(0, 0.02))

#Generator卷积层
#strides:滑动步长  padding: SAME 或者 VALID，指的是不同边缘填充方式
...
def load_examples():
    ...
    print("split Start: " + str(splitStart))

    #add by Tony: split image to small one (256 * 256)
    img = cv2.imread(a.input_file)

    #切割图片为17 X 8 个小图片，每个大小为265X265像素，放入到待验证的文件夹中
    for i in range(17):
       for j in range(8):     
         clp = img[(j * 64):(j * 64 + 256), (i * 64):(i * 64 + 256)]
         name, _ = os.path.splitext(os.path.basename(a.input_file))
         destfile= name+ "_c" + str(i).zfill(2) + "r" + str(j).zfill(2) + ".jpg"   
         print(destfile)
         cv2.imwrite("facades/val/" + destfile, clp)  

    global  splitStop  
    splitStop = time.time()
    print("split Stop: " + str(splitStop))

    #input_paths = glob.glob(os.path.join(a.input_dir, "*.jpg"))
    input_paths = glob.glob(os.path.join("facades/val/", "*.jpg"))
    decode = tf.image.decode_jpeg
    if len(input_paths) == 0:
        input_paths = glob.glob(os.path.join(a.input_dir, "*.png"))
        decode = tf.image.decode_png #图像解码  
    ...
    #如果图像文件名里有数字的话，按照数字进行排序，而不是按照字母排序
    ...
    #在tensorflow中，计算图是从一个管道中读取数据的，为了保证多线程的时候从一个管道读取数据不会乱，就需要线程管理的相关操作。
...
def create_model(inputs, targets):
    ...
    with tf.name_scope("generator_train"):
        with tf.control_dependencies([discrim_train]):
            ...
            #AdamOptimizer函数是Adam优化算法：是一个寻找全局最优点的优化算法，引入了二次方梯度校正。
            #相比于基础SGD算法，1.不容易陷于局部优点。2.速度更快
            #a.lr adam初始学习率，默认是0.0002
            #a.beta1 momentum term of adam ，默认是0.5
            # Create an optimizer.
            gen_optim = tf.train.AdamOptimizer(a.lr, a.beta1)

            #可以通过minimize()函数来同时计算梯度并更新该梯度所对应的参数状态（其计算的就是对应的参数的梯度），
            #若想先计算梯度，然后再将梯度对应参数状态更新，可以通过以下几步来使用：
            #1. 利用 compute_gradients() 函数先计算梯度
            # Compute the gradients for a list of variables.
            gen_grads_and_vars = gen_optim.compute_gradients(gen_loss, var_list=gen_tvars)
            #2. 调用apply_gradients() 函数来更新该梯度所对应的参数的状态。这样有一个好处是，梯度可以按照自己的需求来改动后再使用。
